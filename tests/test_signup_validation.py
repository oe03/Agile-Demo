...
@pytest.mark.parametrize(
    "contact_number,should_be_valid",
    [
        ("", False),  # empty
        ("   ", False),  # whitespace only
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "admin",
            True,
        ),  # currently accepted by backend (flags a gap: should admin be blocked here?)
        (
            "Jobseeker",
            True,
        ),  # different casing, currently accepted (backend doesn't normalize case)
